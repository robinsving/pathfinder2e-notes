---
title: Hammer Of Forbiddance
draft: true
noteType: "hazard"
aliases:
foundryId: Actor.
tags:
  - magical
  - mechanical
  - trap
Source: Pathfinder GM Core
---

# Hammer Of Forbiddance
#Hazard11 
#magical #mechanical #trap 

**Stealth** DC 20 (expert)
**Description** An enormous hammer at an edifice's entrance swings down in an attempt to damage a creature entering an area, push it back, and prevent it from going any further.
***
**Disable** DC 28 thievery (expert) once on the hammer itself and once on its joint to prevent the hammer from swinging
___
**Forbid Entry** `pf2:r`
#attack #divine 
**Trigger** A creature attempts to enter through the entrance.
**Effect** The hammer swings down, making a Strike against the triggering creature.
**Hammer**: `pf2:1`
Strike 28 6d8+20 bludgeoning 
On a hit, the target is knocked back 10 feet and must succeed at a DC: 30 will or be unable to enter the edifice through any entrance for 24 hours (on a critical hit, the target automatically fails the Will save); no multiple attack penalty
***
**Reset** The trap resets over the course of the round, and is ready to swing again 1 round later.