---
title: Vorpal Executioner
noteType: "hazard"
aliases:
foundryId: Actor.
tags:
  - mechanical
  - trap
Source: Pathfinder GM Core
---

# Vorpal Executioner
#Hazard19 
#mechanical #trap 

**Stealth** DC 33 (expert)
**Description** A wickedly sharp saw blade descends and travels along grooves in a complex path throughout the room, attempting to decapitate everyone within.
***
**Disable** DC 41 thievery (expert) at four different junctions to jam all the saw blade's possible paths, preventing it from traveling through the room
___
**Total Decapitation** `pf2:r`
#attack #death 
**Trigger** A creature attempts to exit the room.
**Effect** The saw blade travels along its path, making one Strike against each creature in the room, twisting and varying its height for a maximum chance of beheading its targets.
___
**Decapitation** passive
On a critical hit, a target must succeed at a DC 39 fortitude save or be decapitated, dying instantly unless it can survive without a head.
___
**No Map** passive
The saw blade Strikes do not apply a MAP.
___
**Saw Blade**: `pf2:1`
Strike 40 6d12+25 slashing 
***
**Reset** The trap resets over the course of the round and can be triggered again 1 round later.