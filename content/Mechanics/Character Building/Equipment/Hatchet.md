---
title: "Hatchet"
noteType: ":luggage:"
aliases: "Hatchet"
foundryId: Item.7nKZQ1qM6Wzxaf3v
tags:
  - Item
---

# Hatchet

[Agile](https://2e.aonprd.com/Traits.aspx?ID=170) [Sweep](https://2e.aonprd.com/Traits.aspx?ID=194) [Thrown  10 ft.](https://2e.aonprd.com/Traits.aspx?ID=195)   
**Source** [_Core Rulebook pg. 280_](https://2e.aonprd.com/Sources.aspx?ID=1) [4.0](https://2e.aonprd.com/Sources.aspx?ID=1)  
**Favored Weapon** [Immonhiel](https://2e.aonprd.com/Deities.aspx?ID=238), [Likha](https://2e.aonprd.com/Deities.aspx?ID=192), [Narriseminek](https://2e.aonprd.com/Deities.aspx?ID=144)  
**Price** 4 sp; **Damage** 1d6 S; **Bulk** L  
**Hands** 1  
**Type** Melee; **Category** Martial; **Group** [Axe](https://2e.aonprd.com/WeaponGroups.aspx?ID=1)  

---

This small axe can be used in close combat or thrown.

## Traits

**Agile:**

The multiple attack penalty you take with this weapon on the second attack on your turn is –4 instead of –5, and –8 instead of –10 on the third and subsequent attacks in the turn.

**Sweep:**

This weapon makes wide sweeping or spinning attacks, making it easier to attack multiple enemies. When you attack with this weapon, you gain a +1 circumstance bonus to your attack roll if you already attempted to attack a different target this turn using this weapon.

**Thrown  10 ft.:**

You can throw this weapon as a ranged attack, and it is a ranged weapon when thrown. A thrown weapon adds your Strength modifier to damage just like a melee weapon does. When this trait appears on a melee weapon, it also includes the range increment. Ranged weapons with this trait use the range increment specified in the weapon’s Range entry.

  

## Critical Specialization Effects

**Source** [_Core Rulebook pg. 283_](https://2e.aonprd.com/Sources.aspx?ID=1) [4.0](https://2e.aonprd.com/Sources.aspx?ID=1)  
Certain feats, class features, weapon runes, and other effects can grant you additional benefits when you make a Strike with certain weapons and get a critical success. This is called a critical specialization effect. The exact effect depends on which weapon group your weapon belongs to, as listed below. You can always decide not to add the critical specialization effect of your weapon.  
  
**Axe**: Choose one creature adjacent to the initial target and within reach. If its AC is lower than your attack roll result for the critical hit, you deal damage to that creature equal to the result of the weapon damage die you rolled (including extra dice for its _[striking](https://2e.aonprd.com/Equipment.aspx?ID=280)_ rune, if any). This amount isn't doubled, and no bonuses or other additional dice apply to this damage.
